...
Two separate defects sit outside the trust boundary. `core/tools/edit.ts:466` and `core/tools/write.ts:334` overwrite the user's file through a direct `writeFile`, while `packages/agent/src/harness/session/jsonl/storage.ts:33` publishes the project's own session files through a temp-file-and-rename helper. `core/tools/bash.ts:54` declares `timeout` optional with the description "no default timeout", and `core/tools/bash.ts:281` only arms a timer when the model supplied one.

On Windows, `cli.ts:112` routes every session command into the sandbox path, `core/sandbox/cli-supervisor.ts:33-36` selects the macOS or Linux backend, the Linux backend self-reports unavailable off Linux, and `core/sandbox/supervisor.ts:71` raises `SandboxUnavailableError`. `cli-supervisor.ts:68` prints the reason with no remediation. Neither `README.md` nor `docs/user-guide.md` states that a Windows session requires `--sandbox danger-full-access` to start.
...
- [ ] The reproduction script exits zero. A directory whose only project resource is `.apex-code/permissions.json`, `.apex-code/permissions.local.json`, `.apex-code/agents/`, or `.mcp.json` requires an explicit trust decision.
...
- [ ] This does not add a coverage gate, though `packages/agent/vitest.harness.config.ts:18` scopes coverage to `src/harness/**` plus two files and no workflow invokes it, leaving `packages/coding-agent` unmeasured. That is real and it is not a security boundary. Folding it in would make one spec carry two unrelated arguments. It belongs in its own follow-up.
...
| --- | --- | --- |
| Project resource registry | New. Entries for `settings.json`, `extensions`, `skills`, `prompts`, `themes`, `SYSTEM.md`, `APPEND_SYSTEM.md`, `permissions.json`, `permissions.local.json`, `agents/`, root `.mcp.json`, and the `AGENTS.md` and `CLAUDE.md` candidate list | new module under `core/` |
| Classifier | Derives from the registry. Handles a root-relative entry, not only `.apex-code` children | `core/trust-manager.ts` |
...
The classifier must ask whether a resource confers authority, not whether a file exists. This repository is the proof. `.apex-code/permissions.json` is tracked here and `.gitignore:87-88` records that the `project` source is versioned deliberately, so a presence-only rule would prompt on every clone of Apex Code itself for a file whose content is `{}`. A prompt that carries no decision is how users learn to dismiss prompts, which costs more than the rule gains.
...
Parsing a file in order to decide whether to trust it is the obvious objection, and the answer is the direction of the failure. Reuse the existing `parseScope` rather than introducing a second reader, and treat every parse failure as authority-conferring so a malformed or hostile file prompts rather than passing. Resolve the classification and the load from one read so a file cannot change between the two.
...
Four checklist items are settled by public evidence and only need ticking. `apex-code@0.0.6` carries two registry attestations, including a SLSA v1 provenance whose subject is `pkg:npm/apex-code@0.0.6` and whose build identity is `refs/tags/v0.0.6` in this repository through `.github/workflows/release.yml`. A token publish cannot produce that artifact, so Trusted Publishing is configured for this exact repository and workflow. The `v0.0.6` GitHub Release carries all six named archives plus `SHA256SUMS`. Dependabot is demonstrably opening pull requests, which settles alerts and the dependency graph.
...
| In-place source overwrite at `core/tools/edit.ts:466` and `core/tools/write.ts:334` | behavior | superseded by atomic publish |
...
| A default `bash` timeout kills a legitimate long build | A previously working command times out | Choose the default from observed long runs in this repository's own suite, which takes roughly twenty minutes, and keep the background shell the documented answer for genuinely unbounded work |
