...
**A path can be inside the workspace and still be refused by the executor.** `writePreparedPath` walks from the filesystem root and aborts on any intermediate that is not a real directory, which is deliberate and predates this change. So a workspace reached through a symlink passes the predicate and fails the walk. macOS makes this concrete, because `os.tmpdir()` lives under `/var`, itself a symlink to `/private/var`. The two layers answer different questions, the gate asks whether the call may proceed and the executor asks whether the target it was authorized for is still the target on disk, and the executor is entitled to be stricter. Named here because a test that writes under a temp directory on macOS will meet it and should not mistake it for a permission decision.
