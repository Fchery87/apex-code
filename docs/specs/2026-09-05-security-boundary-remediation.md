...
| Trust context | Resolve trust before constructing project permission stores, MCP runtimes, hooks, and policy loaders. Pass an immutable trust and policy snapshot to the child. | `packages/coding-agent/src/core/trust-manager.ts`, `permissions/`, `mcp/`, `hooks/`, `settings-manager.ts` |
...
| Release identity | Retain the tested tarball and digest, publish those exact bytes, verify the downloaded digest and signed provenance, and remove the second writer. | `.github/workflows/`, `scripts/apex/`, `.github/workflows/build-binaries.yml` |
