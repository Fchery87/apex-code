...
**Apex forks `pi-coding-agent` and `pi-agent-core`. It consumes `pi-ai` and `pi-tui`
as ordinary versioned dependencies.**

The forked half is exactly the half Apex changes. Permissions, sandboxing,
...
ours. Forking it would mean paying that cost forever to gain nothing Apex wants.
Apex's provider work — credential pooling, model roles, fallback chains, measured
...
Apex has no quarrel with it.

Consequences, accepted deliberately:

- Apex is bounded by `pi-ai`'s public API. If something genuinely requires a change
...
- `pi-agent-core` depends on `pi-ai` by semver range. Apex's fork of agent-core
