...
| 2b.7 Fix `ubuntu-latest` CI so the Linux sandbox suite actually runs (bubblewrap install + the RTM_NEWADDR gap 2b.5 surfaced and deferred) | Done | `b9a7bb337` | Real `ubuntu-latest` CI spike (`.github/workflows/linux-bwrap-netns-spike.yml`, deleted after use) isolated the cause and confirmed the fix: `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0` before `bwrap`. With it, `test/sandbox/` on real `ubuntu-latest`: 10 files, 29 passed, 4 skipped (macOS-only tests, correctly gated), 0 failed. `ci.yml` updated to install `bubblewrap` and set the sysctl; confirmed a second time in the real CI matrix (not the throwaway spike): `ubuntu-latest` job green end-to-end, 240 test files / 2114 tests passed, 53 skipped, 0 failed. See notes below. |
