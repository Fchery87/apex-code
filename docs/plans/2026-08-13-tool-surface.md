...
| 4.2 Registry and contract test expansion | In progress — `todo_write`'s registration already extends `contract.test.ts` and `gate-universal.test.ts`'s exhaustive enumeration (invariants 1 and 2); remaining tools still to land and be enumerated | — | Registry factories, `ToolName`, enumeration, representative params, and universal gate cover every Phase 4 tool; each contract has all four axes |
