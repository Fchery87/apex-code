...
| 6.2 Define additive SQLite schema and migrations | Done | `db9fe82e8` + `__TEST_COMMIT__` | Versioned sidecar schema, idempotent reopen, forward-version refusal, explicit non-secret tables, and JSONL readability without a sidecar. `test/durable-state/sqlite-store.test.ts`: 4 passing. |
