...
| --- | --- | --- | --- | --- |
| 12.1 | Settle the credential/state ownership and sandbox handoff contract in an ADR. Define host-owned versus ephemeral state, allowed environment, credential projection, cleanup, and session persistence without widening mounts. | — | complete | `ADR 0015`; verified 2026-08-16 |
| 12.2 | Settle supervisor trust-policy precedence in an ADR. Ensure untrusted project settings cannot influence mounts, network policy, command/args, credentials, or executable resolution before trust; preserve ADR 0005 fail-closed behavior. | — | complete | `ADR 0016`; verified 2026-08-16 |
| 12.3 | Implement and test the credential/state handoff at the public CLI boundary. Prove sanitized environment construction, no forbidden host-path access, no secret/session leakage, and normal/crash cleanup on Linux/macOS; preserve Windows portability behavior. | 12.1, 12.2 | partial — handoff and sanitized environment landed; end-to-end credential turn and cleanup remain | focused sandbox suites passed; full handoff closure pending |
| 12.4 | Implement and test trust-first supervisor policy resolution. Add malicious project fixtures and explicit trusted-policy coverage; prove project settings cannot widen pre-child security authority. | 12.2 | complete | focused launch suite passed; verified 2026-08-16 |
