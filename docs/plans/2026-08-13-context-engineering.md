...
| 3.5 Phase gate verification against the replay corpus | Done — found and fixed a real gap: `replay()` never installed the pipeline at all | `262599f6c` (fix), `69e46088d` (docs) | Fixed `replay()`/`replayCorpus()` constructing a bare `Agent` that bypassed `AgentSession`'s pipeline wiring entirely (confirmed via measurement: `long-tool-heavy` turn-20 was unchanged at 15,272 before the fix). Extracted shared wiring into `core/context/pipeline.ts`. Measured for real: `long-tool-heavy` turn-20 **1,769** (88.4% drop, beats the ≥80% goal); `turnsCompleted` and replay equality unchanged across all nine fixtures. The median-≤670 criterion was retired as mathematically unreachable — see the roadmap's second 2026-08-13 correction — not because it wasn't met, but because it's provably invariant at 1,117 |
| 3.6 Cache measurement and the ordering decision | Done — found the comparison is structurally unmeasurable, not merely unmet | `70d476c64` | Measured `long-tool-heavy` with eviction disabled (budget 10,000,000) vs. the real budget: `cacheHitRate` is byte-identical (0.8569384835479256) either way, because it's computed from each fixture's recorded usage, fixed at authoring time — the offline replay harness cannot simulate a live cache reacting to eviction. No ADR: an ADR fires on a contradicting measurement, and none is possible here. `contracts.md` § 2, the roadmap's Risks, and the spec's Risks/Verification sections are corrected to record this as a structural decision, not a measured one; real cache validation is a stated follow-up |
