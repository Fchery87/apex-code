...
**Status:** Active — approved, implementation not started
...
| --- | --- | --- | --- |
| PGT.1 | Establish the gold-neutral theme contract | Not started | — |
| PGT.2 | Replace the bordered composer with the borderless slab | Not started | — |
...
4. Re-run the narrow theme test, then `npm run typecheck`.
...
5. Run the custom-editor suite, `npm run typecheck`, and `git diff -- packages/tui`.
...
4. Run the footer suites, then the relevant interactive-layout tests and `npm run typecheck`.
...
5. Run the narrow suite, related tool/render regression tests, `npm run typecheck`, and the frozen-package diff.
6. Commit implementation and the plan-row state together; verify and record the SHA.
