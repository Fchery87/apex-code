...
| 8.2 Cross-projection reconciliation gate | Done | `893c6f340` | **Verified, `test/observability/reconciliation.test.ts`:** a session with two assistant messages, a tool-result usage entry, and a compaction entry sums to exactly the same total (0.2) via `getUsageCostBreakdown()` and via the new `aggregateUsagePerformance()` query module over matching ledger rows; a second test proves two sessions' ledger rows never cross-contaminate an aggregate. **A real finding, verified by reading (not assumed) before writing this test:** production assistant turns *and* compaction/branch-summary calls both route through the same `modelRuntime.streamSimple`-backed `streamFn` — `sdk.ts`'s `Agent` construction wires `streamFn` directly to `modelRuntime.streamSimple`, and `context/pipeline.ts`'s `installContextPipeline` always *wraps* the previous `streamFunction` (`previousStreamFunction(...)` is called in every branch) rather than replacing it — so compaction's `streamFn: this.agent.streamFunction` (`agent-session.ts:3142`) is the same instrumented path. This directly contradicts an earlier draft assumption (never committed) that compaction bypassed the ledger via a raw `pi-ai` `streamSimple`; that assumption was based on reading `test-harness.ts`'s simplified test-only `Agent` construction, not production's `sdk.ts`. Test-level implication: the test constructs entries and ledger rows independently (matching this repo's own established pattern in `agent-session-stats.test.ts` for this class of aggregation test) rather than driving a live multi-turn session through real compaction, since triggering token-threshold compaction live is a separate, heavier integration concern the reconciliation gate doesn't need to re-prove. |
| 8.3 Aggregation module and `apex-code cost` | Done | `9a6badce0` | **Verified, `test/observability/cost-command.test.ts` (10 tests):** `parseCostArgs` handles `--by model\|session\|role` (default `model`) and `--since` as either a relative duration (`7d`/`24h`) or an ISO date, rejecting both unknown dimensions and unparseable dates; `formatCostTable` renders a fixed-width table (or "No usage recorded." for an empty ledger) with cost, sample count, and average ttft/generation; `runCostCommand` reads a seeded ledger end to end and prints the grouped table; a fourth test proves `apex-code cost` is wired at the real CLI dispatch by calling `main(["cost"])` against an `ENV_AGENT_DIR`-pointed scratch directory and asserting the seeded cost appears in output — not just that the module works in isolation. Read-only against the durable-state database; never writes. |
| 8.4 `/session` role and latency | Done | `233f6c66f` | **Verified, `test/interactive-mode-session-command.test.ts` (2 tests), using this repo's established `InteractiveMode.prototype.handleXxxCommand.call(context)` duck-typing pattern (matching `interactive-mode-clone-command.test.ts`), not a full TUI harness:** `/session` now renders "Latency" (avg ttft/generation) and "Roles" (per-role cost + request count) sections, reading `ModelRuntime.listUsagePerformanceSamples()` (new public method) filtered to the current session's id; a second test proves a different session's ledger rows never leak into this session's aggregate; existing Cost/Tokens/Messages sections and their exact values render unchanged, proving additive rather than a rewrite; both sections are omitted entirely (not shown empty) when the ledger has no rows for this session, matching how the existing Cost section only appears when `stats.cost > 0`. `handleSessionCommand` and its `/session` call site both became `async` (no other caller exists; `tsgo --noEmit` clean confirms it). |
| 8.5 OTLP trace export | Done | `f092abe82` | **Scope narrowed on the record, before implementation:** one span per model request attempt — the same unit `SqliteUsagePerformanceStore` already records — not a full turn/tool-call span tree, which would require touching `agent-session.ts`'s tool-call lifecycle (a separate, larger integration this pass did not open). `tool_name` stays in the ADR 0012 allowlist for that future work but is never emitted by this task. **Verified, `test/observability/otlp.test.ts` (6 tests) + `test/observability/otlp-production.test.ts` (2 tests, through real `createAgentSessionServices` wiring):** `buildUsageSampleSpan` emits only allowlisted attribute keys and omits undefined fields rather than null attributes; a real driven turn with no `observability.otlpEndpoint` configured produces **zero** `fetch` calls; with an endpoint configured via `SettingsManager.applyOverrides`, the same driven turn POSTs exactly one well-formed OTLP/HTTP JSON payload to `<endpoint>/v1/traces`, every attribute key checked against the allowlist positively (not by absence of known-bad keys); export never throws even when `fetch` rejects. Export uses global `fetch`, never a direct undici import, so it inherits `http-dispatcher.ts`'s proxy-aware dispatcher. `instrumentAttempt`'s early-return guard was widened from `if (!store)` to `if (!store && !otlpConfig)`, since export must not require the ledger to be configured (structurally independent, even though production always configures both today). |
| 8.6 Footer accessibility and display settings | Done | uncommitted | **Verified, `test/footer-accessibility.test.ts` (7 tests), plus the full pre-existing `footer-width.test.ts` (24 tests total) unaffected:** context pressure now carries a `!`/`!!` text marker at the 70%/90% thresholds, present unconditionally (not behind an opt-in) — the color-only WCAG 1.4.1 failure at the old `footer.ts:154-160` is gone by default; `symbolPreset: "ascii"` swaps `↑↓•` for `^v-` everywhere they appear (stats line, session-name separator, thinking-level separator), verified by asserting **every** codepoint in a full render is ≤ U+007F, not just spot-checking known glyphs; `tokenUsageDisplay` honours `off` (hides token/cost stats, keeps context%/model name), `compact` (unchanged existing abbreviation), and `full` (exact `.toLocaleString()` counts). **A real design correction found by the test, not assumed:** `colorBlindMode` was first implemented as `theme.bold(...)` layered onto the existing color — the test caught that `theme.bold`/`.underline` route through `chalk`, which silently drops styling outside a real TTY (confirmed directly: `chalk.bold("hi")` returns `"hi"` unchanged in this environment), making it an unreliable second channel that would vanish in exactly the piped/CI/automated contexts where reliability matters most. Replaced with a real palette adjustment: the >90% threshold moves from `"error"` to `"accent"` — a distinct registered theme hue outside the red/amber family — under `colorBlindMode`, using the same raw-ANSI `theme.fg()` path already proven to survive stripping. `FooterComponent`'s new third constructor parameter is optional (defaults to today's unicode/non-colorblind/compact behavior), so all 7 pre-existing `new FooterComponent(session, footerData)` two-arg call sites across `footer-width.test.ts` needed no changes. Wired at its one production construction site (`interactive-mode.ts`) by passing `this.settingsManager` directly — it already structurally satisfies the new `FooterAccessibilitySettings` interface — and since `render()` reads settings fresh on every call, a live `/settings` change takes effect on the next render with no change-listener wiring needed, keeping the diff to exactly `footer.ts` + `settings-manager.ts` + the one construction line, matching the plan's own "8.6 touches only these two files" scoping. |
