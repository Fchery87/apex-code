...
| 3 | Atomic edit/write via temp-rename in `writePreparedPath`, remove truncate and `writeFile` fallbacks | done | probe `test/tools/atomic-publish.test.ts` observed failing 2 of 7 with 5 controls passing in both states; a reader holding a descriptor across the publish is the discriminator; ungated fallbacks now route through `writePathAtomically`; `test/tools` 257 passed |
| 4 | ADR 0035 + default bash timeout from measurement | done | 3600s, 2.16x the slowest of three recorded `npm test` runs on this host (921s, 1584s, 1666s); probe `test/tools/bash-default-timeout.test.ts` observed failing 3 of 6; the default moved from the local backend to the tool, so a custom backend is no longer silently unbounded |
| 5 | Probes for the surviving 2026-09-05 rows + criterion correction in that spec | done | ten surviving rows already had public-boundary probes, so `test/security-boundary/findings-2026-09-05.test.ts` binds each row to its test and reads the table out of the research doc; nine rows retired against ADR 0032 with reasons; line 50 reworded to what was verified with the classifier half pointing here |
