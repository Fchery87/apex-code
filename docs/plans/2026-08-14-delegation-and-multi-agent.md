...
| 5.7 Ceiling invariant and phase verification | Done, unverified — needs green CI | `test/permissions/contract.test.ts` now enumerates the complete registry for invariant 4; the bash-parent inherited-rule test, legacy `explore:*` grammar compatibility, static-prefix budget, delegation suites, and real Linux network-boundary test are green. Retrieval requires the originating agent type and verifies the handle ownership before returning a result. The full `npm test` run remains red: 12 failures across 9 files, including timing/TUI and path-with-space subprocess failures; the two directly inspected failures also reproduce at baseline `5999e54cd`. Phase completion still needs a clean CI/full-suite run or individual baseline traces for all failures. |
