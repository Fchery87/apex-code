...
**Status:** In progress — tasks 5.1–5.6 implemented; task 5.7 verification remains
...
| 5.3 Recursion depth guard | Done | — | `delegationDepth?: number` added to `SessionHeader`/`NewSessionOptions` (absent means 0; additive-optional, no version bump, per `contracts.md` §3), threaded through `newSession()`'s header construction; `SessionManager.getDelegationDepth()` reads it back (`?? 0`). 4 tests (`test/session-manager/delegation-depth.test.ts`), including a session created with no options at all -- exactly what every pre-5.3 session looks like -- reading back as depth 0. `SettingsManager.getDelegationMaxDepth()`: default 2, clamped to a hard cap (`DELEGATION_MAX_DEPTH_HARD_CAP = 5` -- a conservative placeholder, not a measured number; the plan's own open question about the real cap is still open and carried forward, since no real per-level token-cost measurement exists yet). Non-positive/malformed configured values fall back to the default rather than blocking startup (this is a safety ceiling, not a value worth failing a session over). 5 tests in `test/settings-manager.test.ts`. `runDelegation()` (runtime.ts) checks `getDelegationDepth() >= maxDelegationDepth` right after agent-type resolution and before the capability check -- refused before `buildChildSession` is ever called, so no child session (and, once 5.4 lands, no artifact directory) is created past the bound; the refusal names the configured bound. `BuildChildSessionRequest` gained `depth: number` (parent + 1), assigned by the runtime, not the tool -- 4 new unit tests in `test/delegation/runtime.test.ts` (depth passed correctly, refused at the bound naming it, refused past the bound too, still admitted one level below it). `sdk.ts` wires `getDelegationDepth: () => sessionManager.getDelegationDepth()` and `maxDelegationDepth: settingsManager.getDelegationMaxDepth()` into the real runtime, and records the child's depth via its child `SessionManager` (`inMemory` before task 5.4; the per-child persistent artifact root afterwards); also fixed a real gap task 5.2 left: the recursive `createAgentSession` call inside `buildChildSession` did not forward `options.delegation`, so a child could never delegate again regardless of depth -- silently capping recursion at one level. A real 3-level end-to-end test (`test/delegation/end-to-end.test.ts`) proves the wiring: root (depth 0) delegates to a self-delegating agent, admitted to depth 1, admitted again to depth 2, and depth 2's own delegate attempt is refused before a fourth session is built -- asserted via the faux provider's real (non-cycling, `shift()`-based) call counter landing at exactly 6, not 7. Verified this test is not vacuous: temporarily removed the depth check, confirmed the call count went to 7 and the assertion failed, then restored it. All affected suites green, typecheck clean |
| 5.4 Artifact isolation | Done | — | Per-child `delegations/<uuid>/` root is constructed beneath the parent session directory before child construction, and its persistent `SessionManager` writes the child JSONL there. Tests verify the artifact root exists and an end-to-end permitted `write` still edits the workspace. Worktree isolation remains unmeasured and deliberately does not land. |
| 5.5 Agent definitions and discovery | Done | — | `core/delegation/agents.ts` discovers valid markdown/frontmatter definitions from user scope, and project scope only when `SettingsManager.isProjectTrusted()` permits it. Malformed/incomplete definitions are skipped; a name for an unknown tool is refused by the runtime. |
| 5.6 Background execution and result retrieval | Done | — | `delegate` accepts `{ background: true }` and returns an in-process handle; `{ handle }` awaits either running or completed work. Unknown handles fail explicitly. Its existing deferred schema is retained. |
| 5.7 Ceiling invariant and phase verification | Done, unverified — needs green CI | `test/permissions/contract.test.ts` enumerates the complete registry for invariant 4; the bash-parent inherited-rule test, legacy `explore:*` grammar compatibility, static-prefix budget, delegation suites, and real Linux nested-child network-boundary test are green. Retrieval requires the originating agent type and verifies handle ownership before returning a result. A second `npm test` run (`2026-08-14`) found 15 failures across 11 files, none on the delegation path. Baseline audit against `5999e54cd` reproduced the edit-render failures, host-skill diagnostic, `~`-skill expansion failure, compaction expectation mismatch, model-reload failure, and NO_COLOR/FORCE_COLOR stderr contamination; isolated oversized-image and max-string session-file timeout tests passed, identifying full-suite resource contention. The remaining CLI/external-editor/startup failures are caused by the checkout path containing spaces (`/home/nochaserz/Documents/Coding Projects/...`) or by the baseline worktree's sandbox dependency resolution, and the relevant application files are unchanged since baseline. Phase completion still requires a clean CI/full-suite run or tracked upstream/environment resolution for these harness failures. |
