...
| 9.4 Cross-platform release verification | Done | `82e3b6ed7` | A new `verify-macos-install` job runs on `macos-latest`, `needs: publish` (never re-runs the build/test/publish steps — a job that verified an install by also re-publishing would be a real hazard for a workflow with `id-token: write`), consumes the version via a new `publish` job output (`outputs.version`), and reuses the exact same registry-propagation retry loop as the Ubuntu job (duplicated intentionally, not extracted — the working Ubuntu step stays untouched to avoid any risk to the actual publish path). **Verified, extending `scripts/release-workflow.test.mjs` (1 new test) rather than only trusting YAML validity:** the new job depends on `publish`, runs on `macos-latest`, its commands contain no `npm publish` anywhere, and the whole-file count of `npm publish --access public --provenance --tag next` stays at exactly 2 (unaffected by the new job) — a real regression guard for a pipeline this session cannot execute end-to-end itself. `python3 -c "import yaml"` confirms valid syntax; `npm run check:pinned-deps` confirms the new `actions/upload-artifact`/`actions/setup-node` step pins pass this repo's own SHA-pinning check. Full `test:scripts`: 18/18. |
| 9.5 User documentation and README correction | Done | uncommitted | `README.md`'s status line and its whole "Why" feature list were stale — every landed-since-Phase-1 feature was still marked `*(planned, Phase N)*`, and the "no permission system, no sandbox" paragraph read as describing Apex Code today rather than upstream Pi's starting point; both corrected, with Phases 4–8 added to the list (previously absent entirely). New `docs/user-guide.md`: install, first run (the now theme-only setup dialog from 9.1), and core concepts (permissions, sandbox, sessions, cost) — every concrete claim (`/login`, `auth check`, the five permission modes, `/fork`/`/tree`, `--continue`/`--resume`, `apex-code cost`, `/session`) checked against the actual CLI help text and slash-command definitions, not recalled. Linked from the README's intro and documentation table. **Verified:** every link in both files resolves (directories included, not just files — an earlier pass with `[ -f ]` silently missed `docs/adr/`/`docs/specs/` before being corrected to `[ -e ]`); repo-wide link check afterward stays clean. |
