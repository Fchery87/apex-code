...
**Status:** Active — C and L committed; F changes staged in the tree; root `npm test` and closure open
...
| C.5 Closure: gates, ADR 0015 amendment, changelog, spec status | In progress — ADR/changelog/spec landed in `4016794c3`; `npm run check` green; root `npm test` deferred to the end of all tasks | — | Narrow suites, `npm run build`, `npm run check`, root `npm test`. ADR 0015 gains a dated amendment (the child can now request constrained writes; the read-only handoff itself is unchanged). CHANGELOG `[Unreleased]` entry. Spec status Draft → Implemented. Plan deleted. |
| L.1 Supervisor launch cost: measure, cut `settings-manager` from the launch path | Done | `93e0138ba` | A/B on one loaded machine (load ~23): settings-manager import 1968.8ms → 576.8ms, whole supervisor path 2728.9ms → 812.1ms; calmer runs 196.8ms → ~110ms. Fix: lazy `require` of `proper-lockfile` on first settings write (interface stays synchronous), and the idle-timeout default/parser moved to `core/http-idle-timeout.ts` with `http-dispatcher` re-exporting it, so settings parsing no longer loads `undici`. No settings-parsing logic duplicated. `test/settings-manager.test.ts` (45) and `test/http-dispatcher.test.ts` green. |
| F.1 Verify the `.apex-code` gitignore carve-out is complete | Verified — nothing to change | `33c6f99cd` | `git ls-files .apex-code` lists only intentionally-versioned paths; no runtime path tracked or dirty after a run. Expected no-op — the carve-out landed in `33c6f99cd`; this task exists to verify and record it. |
| F.2 `7209` breadth: prove the harness scrub covers every provider credential variable | Done, commit pending | — | Enumerate the credential env vars the provider registry actually reads (pi-ai `env-api-keys`); a test pins that the suffix/name list in `test/suite/harness.ts` covers all of them, so a new provider's key is covered the day it is added. |
| F.3 `web_search` snippet word-boundary trim | Done, commit pending | — | Red: a highlight longer than the cap ends mid-word today. Green: trim to the last word boundary, test on the adapter seam. |
