...
| TR.6 | Add permission-safe `workspace_symbol` | Done | `cc9264873` | New `test/lsp/workspace-symbol.test.ts` covers workspace-root authorization, normalization, caps, malformed/empty responses, unsupported servers, cancellation, outside-root results, and the canonical contract projection; full `npm test` (389 files / 3275 tests passed) and `npm run check` green on the tree carrying this row |
