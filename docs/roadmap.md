...
| Trust classification and proof integrity | **active** · the classifier bypass is closed and shipped in `0.1.0` at `a2cd363ab`: `hasTrustRequiringProjectResources` derives the gated set from one registry the loaders resolve through, so the four project-controlled resources that skipped the prompt no longer can. Branch protection is on. Remaining: `projectTrusted` required across loaders (needs an ADR), gating `AGENTS.md` and `CLAUDE.md`, probes for the surviving 2026-09-05 rows, atomic `edit`/`write`, and a default `bash` timeout | [spec](specs/2026-09-11-trust-classification-and-proof-integrity.md) | [plan](plans/2026-09-11-trust-classification-and-proof-integrity.md), [completion](plans/2026-09-19-active-workstream-completion.md) |
