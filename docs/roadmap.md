...
| Criterion | State |
| --- | --- |
| CI green on three platforms | **partial.** Four runs completed; the frozen-package gate and builds pass, but all three OS legs remain red at `npm run check`, and tests have not run in CI. The latest failure is stale generated lock state for the deleted `evals` and `session-backends` workspaces. Linux is the required gate; macOS and Windows are advisory until Task 0.10 sets their baseline, since upstream CIs Linux only. The criterion is amended accordingly: *Linux green, macOS and Windows characterised.* |
