...
| Chrome consistency | **landed** — one hint-row grammar, one composer-dock edge with the `›` prompt, sentence-case titles, no palette scope letters, `cache N%` and `<0.1%` in the footer, and a one-line update notice by default · `2abf25a18`, `72849daed`, `136a0688a`, `fee66d4a1`, `3b587cfed` · `npm test` green | [spec](specs/2026-09-22-chrome-consistency.md) | — |
