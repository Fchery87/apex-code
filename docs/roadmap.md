...
| Transcript polish | **landed** — one look per role (user spine with a gap, no assistant rail), a failed retry run collapsed to one error line, and every counted tool hint routed through `formatHiddenLines` with bash duration left to the header · `05678522b`, `a3f7f630f`, `99e873e6e` · `npm test` green; the render bench fails at startup on `main` too, recorded in the spec | [spec](specs/2026-09-22-transcript-polish.md) | — |
