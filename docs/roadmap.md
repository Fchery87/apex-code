...
| Bash provider schema | **landed** — the root `anyOf` union left the Anthropic adapter copying an empty object schema, so every `bash` call failed local validation before execution. `command`, `timeout`, `background`, `handle`, and `kill` are now root properties derived from the same field schemas the alternatives use, the union and the inferred `BashToolInput` are retained, and an empty call still fails before the permission hook · pinned by `test/tools/bash-schema.test.ts` | [spec](specs/2026-09-07-bash-provider-schema.md) | — |
| Shell operation boundary | **landed** — one parsed operation shared by permissions, execution, preview, display, and evidence, closing a permission-rule bypass on calls that named two operations at once · `5bbda59ad` reproduced it, `62743a421` and `d50341b97` closed it, merged as pull request #111 at `e9ee3b465` and shipped in `0.2.1` · pinned by `test/tools/shell-operation-boundary.test.ts` | [spec](specs/2026-09-16-shell-operation-boundary.md) | — |
| Responses tool argument recovery | **landed** — local verification complete | [spec](specs/2026-09-18-responses-tool-argument-recovery.md) | — |
...
| Unified runs and child sessions | **landed** — root aggregate budget (explicit `aggregateBudget`, parent consumption, one ledger per tree), child concurrency caps, attempt records with idempotent spawn, timeouts, cancellation reasons, pollable `agent/status`, workspace states with explicit `agent/recover` (no auto recreation), policy/sandbox/artifact/evidence linkage, and token/cost rollups landed as eight slices `8ba92f669` through `34c53156b`, plus the Windows portability repairs `01855109a`, `8d096b119`, `7bfd30934`, and `5473ceea9` · three-OS CI green (run `34562097691` at `5473ceea9`) · the two settings decisions this row carried as open were already settled in the spec's own risk note: the aggregate is opt-in with no default, and a configured aggregate is consumed by the parent's own runs. Whether a default should turn on for every session awaits replay measurements and is a new question, not unfinished phase work | [spec](specs/2026-09-09-run-and-child-session-architecture.md) | — |
...
| Windows prepared paths and CI coverage | **landed** — prepared reads and writes begin at the absolute path's parsed filesystem root, so a C: target executed from a D: checkout no longer resolves as `D:\\C:`; drive, UNC share, and namespaced roots stay intact and the descriptor identity, exclusive-creation, and no-follow guarantees are unchanged · pinned by `test/prepared-path-windows.test.ts` · the platform-specific sandbox assertions went with the boundary on 2026-09-12, see the spec's amendment | [spec](specs/2026-09-08-windows-ci-path-roots.md) | — |
| Ember workflow completion | **landed** — nine of ten goals delivered: selection painting, quiet overlay borders, a composer origin that holds its column across every mode, per-call tool disclosure, a permission prompt that names the scope it grants, and an actionable compaction hint. The tenth, showing the proposed change at approval time, was deferred by this spec on purpose and delivered by the Permission preview row below · frame budget measured trunk `1e0a86c17` against the branch back to back on an idle host, recorded in the spec | [spec](specs/2026-09-07-ember-workflow-completion.md) | — |
| Permission preview | **landed** — the prompt shows the change it is authorizing, read through the `PreparedPathOperation` the gate validated so it cannot describe a different file from the one that will be written; a tool with no producer and a file the producer cannot read both degrade to a truthful summary rather than to silence or a fabricated diff · `c89780918`, `bce1533ac`, `145d5f970`, merged as pull request #93 at `05d7ff099` · pinned by `test/permission-preview-render.test.ts` and `test/permissions/preview.test.ts` | [spec](specs/2026-09-07-permission-preview.md) | — |
| Persist a refusal | **landed** — `persist` now means the same thing on both branches of the gate, so "Reject always" writes a session deny rule from the tool's own `ruleForCall()` instead of refusing once and asking again; a denial without `persist`, and one whose tool yields no rule, still write nothing, and ACP offers `reject_always` only when a rule would be written · pinned by `test/permissions/responder.test.ts` | [spec](specs/2026-09-07-persist-a-refusal.md) | — |
| Trust classification and proof integrity | **landed** — the classifier bypass closed in `0.1.0` at `a2cd363ab`, and the five gaps that row carried as remaining are closed at `f9887fda8`, `6b749dc92`, and `226775ba9`. `projectTrusted` is a required argument on the permission store, the settings manager, and the MCP runtime, validated at runtime as well as in the type ([ADR 0034](adr/0034-project-trust-is-a-required-argument.md)); making it required found that none of the four production call sites had a trust answer at its call point, and that startup read a project's `sessionDir` before any decision existed, which made `sessionDir` a global setting rather than a merged one. `AGENTS.md` and `CLAUDE.md` are gated and registered, closing the product question the earlier slice deferred: a repository whose only project file is an `AGENTS.md` now asks once. `edit` and `write` publish by rename, preserving the destination's mode. `bash` defaults to a one-hour wall-clock timeout derived from recorded suite runs ([ADR 0035](adr/0035-default-bash-timeout.md)). Every 2026-09-05 row has a disposition: ten name a committed probe, nine are retired against ADR 0032, and the index reads the table out of the research document so a new row with no disposition fails. The governance checklist is settled, twelve items verified live with dates and two annotated as needing npm account access · three-OS CI evidence is the operator gate, since this work is unpushed | [spec](specs/2026-09-11-trust-classification-and-proof-integrity.md) | — |
...
| Permission gate recalibration after the boundary removal | **landed** — ADR 0032 deleted the layer two of the gate's defaults assumed, leaving `acceptEdits` auto-allowing a write to any path the account could reach and the read-shaped tools returning the agent directory's `auth.json` with no decision. Both are closed: the auto-allow is confined to targets that resolve inside the workspace, with a dangling link counted as outside, and the credential file is refused on terms no project or local rule can express · `18697ca93`, `6a0070e6f`, `631b2da55` (pull requests #103 and #104, four required checks green on each) | [spec](specs/2026-09-13-permission-gate-recalibration.md) | — |
...
| Collapsed-by-default transcript | **landed** — a session opens at overview with thinking collapsed to one `Thinking... ctrl+o to expand` line and multi-line errors folded to a summary; the level you pick is remembered in settings as `chatDetail`; `ctrl+t` at overview opens to details · `afd8ac67a`, `0d7b2183a`, `37aa71d4a` · `npm test` green (3620 passed, 51 skipped) | [spec](specs/2026-09-25-collapsed-by-default-transcript.md) | — |
