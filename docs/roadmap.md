...
| Shell operation boundary | **active** — one parsed operation shared by permissions, execution, preview, display, and evidence, closing a permission-rule bypass on calls that named two operations at once | [spec](specs/2026-09-16-shell-operation-boundary.md) | — |
...
| Windows prepared paths and CI coverage | **active** · root traversal fix under verification; the platform-specific sandbox assertions went with the boundary on 2026-09-12, see the spec's amendment | [spec](specs/2026-09-08-windows-ci-path-roots.md) | — |
...
| Permission gate recalibration after the boundary removal | **active** · ADR 0032 left the gate's calibration untouched while deleting the layer two of its defaults assumed; `acceptEdits` auto-allows a write to any path the account can reach and the read-shaped tools return the agent directory's `auth.json` with no decision, both reproduced against `6408ab9af` | [spec](specs/2026-09-13-permission-gate-recalibration.md) | [plan](plans/2026-09-13-permission-gate-recalibration.md) |
