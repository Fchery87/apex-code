...
Apex Code publishes stable releases and is still moving fast. Before investing in a large change, open an issue
...
- **Do not patch any frozen package.** `packages/ai`, `tui`, `client`,
  `protocol`, `server`, `telemetry` are consumed, not forked. Extend
