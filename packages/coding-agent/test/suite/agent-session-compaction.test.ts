...
describe("AgentSession compaction characterization", () => {
	...
	it("compacts after a tool result before the next assistant request in the same run", async () => {
		...
		// v0.84.4 asserts one `agent_start`, because upstream compacts inside the run.
		// Apex resumes by restarting the run, so it emits a second one. Both implement
		// #6879; the divergence is recorded in docs/upstream-log.md with the follow-up
		// that decides whether Apex keeps its own implementation.
