...
describe("CLI sandbox supervisor", () => {
	...
	it("holds the session lease with the host supervisor pid while the sandbox child runs", async () => {
		...
		expect(JSON.parse(readFileSync(leasePath, "utf8"))).toMatchObject({ pid: process.pid, cwd: realpathSync(cwd) });
	...
	it.skipIf(process.platform === "win32")(
		"creates a private canonical auth file before launching a fresh sandbox",
		async () => {
			const cwd = workspace();
	...
	it.skipIf(process.platform === "win32")(
		"removes the private credential endpoint when launch fails after proxy creation",
		async () => {
			const cwd = workspace();
			const authPath = join(cwd, "host-agent", "auth.json");
	...
	it.skipIf(process.platform === "win32")(
		"closes the credential endpoint even when backend cleanup rejects",
		async () => {
			const cwd = workspace();
			const authPath = join(cwd, "host-agent", "auth.json");
			let socketPath: string | undefined;
