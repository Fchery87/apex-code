...
```typescript
import { createAgentSession, ModelRuntime, SessionManager } from "apex-code";
...
The `Agent` class (from `apex-code-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.apex-code/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.apex-code/extensions/`)
- Project skills:
  - `.apex-code/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.apex-code/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.apex-code/agent/skills/`)
  - `~/.agents/skills/`
...
import { ModelRuntime } from "apex-code";
...
import { createAgentSession, ModelRuntime } from "apex-code";

// Default: uses ~/.apex-code/agent/auth.json and ~/.apex-code/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "apex-code";
...
import { createAgentSession, defineTool } from "apex-code";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.apex-code/agent/extensions/`, `.apex-code/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "apex-code";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "apex-code";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "apex-code";
...
1. Global: `~/.apex-code/agent/settings.json`
2. Project: `<cwd>/.apex-code/settings.json`
