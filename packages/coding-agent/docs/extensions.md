...
> **Placement for /reload:** Put extensions in `~/.apex-code/agent/extensions/` (global) or `.apex-code/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.apex-code/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.apex-code/agent/extensions/*.ts` | Global (all projects) |
| `~/.apex-code/agent/extensions/*/index.ts` | Global (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `apex-code` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
```typescript
import { isToolCallEventType } from "apex-code";
...
```typescript
import { isBashToolResult } from "apex-code";
...
```typescript
import { createLocalBashOperations } from "apex-code";
...
```typescript
import { SessionManager } from "apex-code";
...
```typescript
import { withFileMutationQueue } from "apex-code";
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "apex-code";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "apex-code";
