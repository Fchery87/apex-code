...
apex-code -r                  # Browse and select a session
apex-code --no-session        # Ephemeral mode; do not save
apex-code --name "my task"    # Set session display name at startup
apex-code --session <path|id> # Use a specific session file or session ID
...
- `.apex-code/SYSTEM.md` for a project
- `~/.apex-code/agent/SYSTEM.md` globally
...
If you use Apex Code for open source work and want to publish sessions for model, prompt, tool, and evaluation research, see upstream Pi's [`badlogic/pi-share-hf`](https://github.com/badlogic/pi-share-hf), which publishes sessions to Hugging Face datasets and works the same way against Apex Code's session format.
...
apex-code update [source|self]      # Update Apex Code itself (self), or one package source
apex-code update --all              # Update Apex Code and packages; reconcile pinned git refs
apex-code update --extensions       # Update packages only; reconcile pinned git refs
...
`update`'s `source` argument also accepts the legacy value `pi` as an alias for `self`, for compatibility.

These commands manage Apex Code packages and `apex-code update` can update the Apex Code CLI installation. To uninstall Apex Code itself, see [Quickstart](quickstart.md#uninstall). `apex-code config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `apex-code update` never prompts for project trust.

See [Apex Code Packages](packages.md) for package sources and security notes.
...
In print mode, Apex Code also reads piped stdin and merges it into the initial prompt:
...
```bash
apex-code --no-extensions -e ./my-extension.ts
```
...
Apex Code inherits Pi's design principle of keeping the core small and pushing
workflow-specific behavior into extensions, skills, prompt templates, and packages.
Anything not built in can be added as an extension or package, or handled with
external tools such as containers and tmux.

For Pi's own full rationale, read its [blog post](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/).
