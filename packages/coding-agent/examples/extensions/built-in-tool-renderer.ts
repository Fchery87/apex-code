...
import type { BashToolDetails, EditToolDetails, ExtensionAPI, ReadToolDetails } from "apex-code";
import { createBashTool, createEditTool, createReadTool, createWriteTool } from "apex-code";
	...
	pi.registerTool({
		...
		renderCall(args, theme, _context) {
			...
			if ("command" in args) {
				...
				text += theme.fg("accent", `${args.handle}${"kill" in args ? " · kill" : ""}`);
			}
			return new Text(text, 0, 0);
		},
