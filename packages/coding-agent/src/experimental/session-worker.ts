...
import type { AgentLane } from "apex-code-agent-core/harness/agent-harness";
import { AgentHarness, type AgentHarness as AgentHarnessInstance } from "apex-code-agent-core/harness/agent-harness";
...
async function createCodingAgentHarness(
	...
	// Source-only experimental path: it resolves no project trust, so it fails closed (ADR 0034).
	const settingsManager = SettingsManager.create(session.metadata.cwd, undefined, { projectTrusted: false });
