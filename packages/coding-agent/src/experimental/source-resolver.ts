...
// Apex keeps explanatory full-line comments in tsconfig.json, which JSON.parse rejects.
const tsconfig = JSON.parse(readFileSync(tsconfigPath, "utf8").replace(/^\s*\/\/.*$/gm, "")) as TsConfig;
...
const aliases: SourceAlias[] = Object.entries(paths)
	.filter(([pattern]) => pattern.startsWith("@earendil-works/") || /^apex-code(?:-agent-core)?(?:\/|$)/.test(pattern))
