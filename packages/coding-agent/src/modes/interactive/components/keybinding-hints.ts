...
/** A binding, or a key that is not one. */
export type HintKeyPart = Keybinding | { literal: string };

/** One key, or several shown as `a/b` by their primary binding. */
export type HintKey = HintKeyPart | readonly HintKeyPart[];

function hintKeyText(part: HintKeyPart, options: KeyTextFormatOptions): string {
	return typeof part === "string" ? keyText(part, options) : formatKeyText(part.literal);
}

/**
 * The one grammar for a row of key hints: `up/down move · enter select · escape close`.
 * Keys come from the live bindings, so a rebound key is reported as the user bound it.
 */
export function hintRow(
	hints: ReadonlyArray<readonly [key: HintKey, action: string]>,
	options: { ascii?: boolean } = {},
): string {
	const separator = theme.fg("borderMuted", options.ascii ? " - " : " · ");
	return hints
		.map(([key, action]) => {
			const text = Array.isArray(key)
				? key.map((part: HintKeyPart) => hintKeyText(part, { primaryOnly: true })).join("/")
				: hintKeyText(key as HintKeyPart, {});
			return `${theme.fg("dim", text)}${theme.fg("muted", ` ${action}`)}`;
		})
		.join(separator);
}
