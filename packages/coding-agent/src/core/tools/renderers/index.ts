...
/**
 * The built-in tools that ship a renderer. Apex registers more built-in tools than upstream
 * (ask_user, delegate, and others), and those render through the generic fallback, so this is a
 * subset of `ToolName` rather than all of it.
 */
...
export function createAllToolRenderers(): Record<RenderedToolName, ToolRenderers> {
	return {
		read: readRenderers,
...
export function withBuiltInRenderers<TDefinition extends ToolRenderers>(
	...
	const builtIn: ToolRenderers | undefined = createAllToolRenderers()[toolName as RenderedToolName];
