...
- **A session opens with everything collapsed.** Tool output shows a counted preview, edit diffs show their line counts, thinking shows one `Thinking... ctrl+o to expand` line, and a multi-line error shows its first line (or, for a Python traceback, the raised error) with the same hint. Press `ctrl+o` to step through overview, details, and all. The level you pick is saved as `chatDetail` in `~/.apex-code/settings.json`, so the next session opens at it; `/settings` also has a Conversation detail row. Turning thinking on with `ctrl+t` opens the transcript far enough to show it.
...
- **`apex-code mcp auth` read the project's `.mcp.json` by a path the classifier does not resolve.** It composed the path itself rather than going through the project resource registry, so a `piConfig.configDir` override would have pointed the command at one file while the trust classifier checked another. It now resolves through the registry like every other reader of a gated resource. The command is deliberately not trust-gated: naming one server to authorize is the user's decision, and a one-shot command has no session trust state to consult. The guard that was supposed to catch this shape could not see it, because it matched the filename as a string and the two offending sites reached it through a constant; it now checks the identifiers too, and was proved by reintroducing the old line and watching it get named.

- **`edit` and `write` could destroy a file they failed to replace.** Both truncated the authorized target and then wrote into it, so the window between the two held a file with none of the old content and not yet all of the new. A crash, a full disk, or a kill in that window left the only copy gone. They now write a sibling and rename it over the target, which is what the session store has always done, so the name refers to the complete old file until it refers to the complete new one. The destination's mode is carried across, so publishing no longer resets permissions. The device-and-inode identity check that makes the authorized target the written target is unchanged, and a new-file write still proves the name was absent before it publishes. One consequence is worth stating: a rename installs a file this process created, so a target owned by someone else and merely writable by you changes owner. On Windows, where a rename onto a destination another handle holds open is refused and an ordinary read handle is enough, the write falls back to the previous in-place path rather than failing, so a file open in an editor still writes. That write is not crash-safe, which is the honest cost of the platform refusing the atomic one.

- **A `bash` command with no `timeout` ran unbounded.** A process waiting on input nothing will send, or a network call with no timeout of its own, held the tool call open for the life of the session, and nothing in the loop could tell that apart from slow. Calls now default to one hour, a value derived from this repository's own recorded suite runs rather than from taste, and the message when it fires names both ways forward: a larger explicit `timeout`, or the background shell. A background launch is still unbounded, because it is the escape hatch. See ADR 0035.

- **A cloned repository's `AGENTS.md` reached the system prompt with no trust decision.** Repository instruction files are wrapped as `<project_instructions>` and handed to the model as instructions, so a checkout supplying one is steering the agent, which is the thing the trust prompt exists to ask about. They were loaded before any decision and were not among the resources whose presence raises one. A test asserted the old behavior, which is how it survived review. An untrusted checkout's `AGENTS.md`, `AGENTS.override.md`, `AGENTS.MD`, `CLAUDE.md`, and `CLAUDE.MD` are now withheld, along with any ancestor directory's, while the agent directory's own still loads because it is yours and not the project's. This is not prompt-injection defense and does not claim to be; it puts the highest-value injection surface in the product behind the same decision every other project-controlled surface is behind. The cost is a first-run change: a repository whose only project file is an `AGENTS.md` now asks once, where it used to start silently, and most repositories have one.

- **A project could choose where its own session transcripts were written, before you trusted it.** Startup builds a settings manager before the trust store exists, and that manager read the merged global-and-project settings, so a cloned repository shipping `.apex-code/settings.json` with `sessionDir` set had that path honored with no trust decision anywhere in the sequence. A transcript carries everything the session saw, so choosing where one lands is an exfiltration primitive that needs no cooperation from the model: point it somewhere the repository reads or commits and the contents follow. `sessionDir` is now a global setting, like `defaultProjectTrust`, and a project's value is ignored whether or not you trust the project. Your own `sessionDir`, the `--session-dir` flag, and `~` expansion are unchanged. This is a behavior change for anyone who set it per project, and the reason it is a removal rather than a repair is that there is no point in startup where honoring a project's value would be both safe and possible.
...
- **An allowed command could carry a denied background kill past the permission gate.** The shell tool advertises one flat set of fields because providers drop the union constraint that would otherwise separate its call shapes, and the consumers reading those fields disagreed about which operation a call requested. Permissions answered by whether `command` was present, execution answered by whether `handle` was present, and the evidence ledger answered like permissions. A call carrying both was therefore authorized as its command and performed as its handle. With `pwd` allowed and `background-handle` denied, adding `command: "pwd"` to `{handle, kill: true}` turned a blocked call into an allowed one, sent the kill, and recorded `pwd` in the evidence ledger as the command that ran, so the record named something that never executed. What this reached is background retrieval and termination on a handle already in the session's registry; it did not establish arbitrary new command execution. A call's operation is now parsed once, before anything acts on it, and permissions, execution, the permission preview, the displayed title, and evidence all read that one answer. A call that names two operations, or none, is rejected with a reason naming both ways forward instead of being dispatched to whichever operation the reading consumer happened to prefer. A placeholder the model did not mean to send, such as an empty `handle` beside a real command, is dropped rather than failing the call with `Unknown background handle: .`, so that call now runs its command and is gated as the command it is.
