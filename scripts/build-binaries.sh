#!/usr/bin/env bash
#
# Build Apex Code binaries for all platforms locally.
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before Apex Code starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/apex-code.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/apex-code"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating apex-code-$platform.zip..."
        (cd "$platform" && zip -r ../apex-code-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating apex-code-$platform.tar.gz..."
        mv "$platform" apex-code && tar -czf apex-code-$platform.tar.gz apex-code && mv apex-code "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../apex-code-$platform.zip)
    else
        tar -xzf apex-code-$platform.tar.gz && mv apex-code "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/apex-code.exe"
    else
        echo "  $OUTPUT_DIR/$platform/apex-code"
    fi
done
