...
const packages = [
	...
	{ directory: "packages/agent", name: "apex-code-agent-core" },
	...
	{ directory: "packages/coding-agent", name: "apex-code" },
];
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `apex-code-${platform}.zip` : `apex-code-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "apex-code.cmd"))) {
			writeFileSync(join(installDirectory, "apex-code.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\apex-code.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "apex-code.ps1"), '& "$PSScriptRoot/node_modules/.bin/apex-code.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "apex-code.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\apex-code.exe" %*\r\n');
		writeFileSync(join(installDirectory, "apex-code.ps1"), '& "$PSScriptRoot/node_modules/.bin/apex-code.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "apex-code"), join(installDirectory, "apex-code"));
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `apex-code-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "apex-code.exe" : "apex-code")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "apex-code.cmd" : "apex-code")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "apex-code.cmd" : "apex-code")} --help`);
	}
}
